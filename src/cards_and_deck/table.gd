...
func _ready():
	...
	# --- MODIFICATION START ---
	# Connect signals from the decks programmatically.
	...
	if elemental_deck:
		# Disconnect any potentially incorrect connections made in the editor
		...
		# Now, ensure the correct signal is connected.
...
func plant_initial_elemental_cards():
	...
	for i in range(min(8, selected_indices.size())):
		var card_resource = elementals_cards.cards[selected_indices[i]]
...
func reset_available_cards():
	available_cards = []
	elemental_cards = []
	# Populate action cards from the actions_cards resource
	for i in range(actions_cards.cards.size()):
		available_cards.append(i)
		
	# Populate elemental cards from the elementals_cards resource
...
func add_elemental_card():
	...
	if data:
		...
		# Elemental cards don't affect turn phases in the same way, so no phase logic here.
		if game.network_manager and game.network_manager.multiplayer and game.network_manager.multiplayer.get_peers().size() > 0:
			game.network_manager.sync_card_drawn(data["card_id"])
		return true
	return false
...
func next_elemental_card():
	...
	print("Selected elemental card from deck: index =", card_index, "resource card_id =", resource_card_id)
	elemental_cards.remove_at(random_index_position)
	print("Drawing elemental card index", card_index, "with resource card_id", resource_card_id, "(", elemental_cards.size(), "cards left)")
	return {
		"id": card_index,
...
# Renamed for clarity. This should be connected to the Action Card deck.
...
# This should be connected to the Elemental Card deck.
